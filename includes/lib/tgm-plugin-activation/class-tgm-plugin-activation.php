 ...
 * @version   2.6.1 for parent theme brikk for publication on ThemeForest
	...
	class TGM_Plugin_Activation {
		...
		public function notices() {
			...
			if ( ! empty( $message ) || $total_required_action_count > 0 ) {
				...
				if ( ! current_user_can( 'activate_plugins' ) && ! current_user_can( 'install_plugins' ) && ! current_user_can( 'update_plugins' ) ) {
					...
					$rendered = '<span class="brk-db-settings-error-heading">' . esc_html__( 'Install / Activation / Update Of Required Plugin(s) Needed', 'brikk' ) . '</span>';
		...
		protected function get_admin_notice_class() {
			if ( ! empty( $this->strings['nag_type'] ) ) {
				return 'brk-db-notice ' . sanitize_html_class( strtolower( $this->strings['nag_type'] ) );
			} else {
				if ( version_compare( $this->wp_version, '4.2', '>=' ) ) {
					return 'notice-error brk-db-notice';
				} elseif ( version_compare( $this->wp_version, '4.1', '>=' ) ) {
					return 'notice brk-db-notice';
				} else {
					return 'updated brk-db-notice';
				}
			}
		}
...
if ( ! class_exists( 'TGMPA_List_Table' ) ) {
	...
	class TGMPA_List_Table extends WP_List_Table {
		...
		protected function get_row_actions( $item ) {
			...
			$prefix = ( defined( 'WP_NETWORK_ADMIN' ) && WP_NETWORK_ADMIN ) ? 'networz_admin_' : '';
