...
class WooCommerce {
    ...
    function __construct() {
        ...
        add_action( 'woocommerce_edit_account_form_start', [ $this, 'add_field_edit_account_start' ] );
    ...
    public function save_account_details( $user_id ) {
        ...
        if( isset( $_POST['account_type'])) {
            // we check that the option exists in get_field_object('field_640368dfda8ce')['choices']
            $account_type = sanitize_text_field( $_POST['account_type'] );
            $allowed_account_types = get_field_object('field_640368dfda8ce')['choices'];

            if( array_key_exists( $account_type, $allowed_account_types ) ) {
                update_field( 'account_type', $account_type, 'user_' . $user_id );
            }
        }

        if (isset($_POST['civilite'])) {
            // we check that the option exists in get_field_object('field_64036a5eeb343')['choices']
            $civilite = sanitize_text_field($_POST['civilite']);
            $allowed_civilites = get_field_object('field_64036a5eeb343')['choices'];

            if (array_key_exists($civilite, $allowed_civilites)) {
                update_field('civilite', $civilite, 'user_' . $user_id);
            }
        }
    }
    ...
    public function add_field_edit_account_start() {
        if( ! function_exists('routiz') ) {
            return;
        }
        ...
        // add account_type field (select) (acf field name: account_type - field_640368dfda8ce)
        $form->render([
            'type' => 'select',
            'id' => 'account_type',
            'value' => get_field('account_type', 'user_' . get_current_user_id()),
            'options' => get_field_object('field_640368dfda8ce')['choices'],
            'name' => get_field_object('field_640368dfda8ce')['label'],
            'allow_empty' => false,
            'required' => get_field_object('field_640368dfda8ce')['required'],
        ]);

        // add civilite field (select) (acf field name: civilite - field_64036a5eeb343)
        $form->render([
            'type' => 'select',
            'id' => 'civilite',
            'value' => get_field('civilite', 'user_' . get_current_user_id()),
            'options' => get_field_object('field_64036a5eeb343')['choices'],
            'name' => get_field_object('field_64036a5eeb343')['label'],
            'allow_empty' => false,
            'required' => get_field_object('field_64036a5eeb343')['required'],
        ]);
    }
